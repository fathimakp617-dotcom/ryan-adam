  ...
  const filterReturns = () => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        (r) => r.order_number.toLowerCase().includes(query) ||
              ...
              filteredReturns.map((returnItem) => (
                <TableRow key={returnItem.id}>
                  ...
                  <TableCell className="text-right">
                    <Button size="sm" variant="ghost" onClick={() => setSelectedReturn(returnItem)}>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="mt-1 p-3 bg-muted rounded-lg text-sm">
                  <p>{selectedReturn.shipping_address.address}</p>
                  <p>{selectedReturn.shipping_address.city}, {selectedReturn.shipping_address.state}</p>
                  <p>{selectedReturn.shipping_address.zipCode}, {selectedReturn.shipping_address.country}</p>
                </div>
              </div>
              ...
              <div className="pt-2 text-center">
                <span className={`px-4 py-2 rounded-full text-sm font-medium ${getStatusColor(selectedReturn.return_status)}`}>
